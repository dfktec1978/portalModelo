      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            {store && (
              <div className="mt-6 text-sm text-gray-400">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <a href={`/lojas/${store.slug || store.id}`} className="w-16 h-16 bg-white/5 rounded overflow-hidden flex-shrink-0 block relative" target="_blank" rel="noreferrer">
                    <Image src={store.logo || store.logo_url || store.image || store.cover || '/img/logos/logo.png'} alt={store.store_name || store.name || 'Loja'} fill className="object-cover" unoptimized />
                  </a>
                  <div>
                    <div className="flex items-center gap-2">
                      <a href={`/lojas/${store.slug || store.id}`} target="_blank" rel="noreferrer" className="font-semibold hover:underline">{store.store_name || store.name || "Sem nome"}</a>
                      <a href={`/lojas/${store.slug || store.id}`} target="_blank" rel="noreferrer" className="text-xs bg-blue-600 text-white px-2 py-1 rounded">Visualizar página</a>
                    </div>
          ...
          <section className="lg:col-span-3">
            <div className="bg-white/5 p-6 rounded-lg">{children}</div>
          </section>
        </div>
      </main>
    </div>
