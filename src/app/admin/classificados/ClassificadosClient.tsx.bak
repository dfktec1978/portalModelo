        ...
        <form onSubmit={handleSave} className="bg-white rounded shadow p-6 mb-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <input placeholder="Título" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="form-input" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input placeholder="Localização" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="form-input" />
            </div>
            <div>
              <label className="form-label">Preço</label>
              <input type="number" step="0.01" placeholder="Preço" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className="form-input" />
            </div>
            <div>
              <label className="form-label">Descrição</label>
              <Editor value={form.description} onChange={(val) => setForm({ ...form, description: val })} placeholder="Descrição" />
            </div>
            <div>
              <label className="form-label">Imagens</label>
              <ImageUploadNews images={form.imageUrls} heroImageIndex={0} onImagesChange={(images) => setForm({ ...form, imageUrls: images })} onHeroImageChange={() => {}} disabled={saving} maxImages={5} />
            </div>
            <div className="flex items-center gap-2">
              <button type="submit" disabled={saving} className="bg-blue-700 text-white px-4 py-2 rounded">
                ...
                <div className="flex gap-4">
                  <div className="flex-shrink-0">
                    {classified.image_urls && classified.image_urls.length > 0 ? (
                      <Image src={classified.image_urls[0]} alt={classified.title} width={80} height={80} className="rounded object-cover" />
                    ) : (
                  ...
                  <div className="flex flex-col gap-2">
                    <select value={classified.status} onChange={(e) => handleStatusChange(classified.id, e.target.value as any)} className="px-3 py-1 bg-gray-100 text-gray-800 border border-gray-300 rounded text-sm">
                      <option value="active">Ativo</option>
                      <option value="sold">Vendido</option>
                      <option value="removed">Removido</option>
                    </select>
                    <Link href={`/classificados/${classified.id}`} className="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm text-center">
                      Ver
                    </Link>
                    <button onClick={() => handleDelete(classified.id)} className="px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded text-sm">
                      Deletar
                    </button>
                    <button onClick={() => (window.location.href = `?edit=${classified.id}`)} className="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm">
                      Editar
                    </button>
                  </div>
                </div>
