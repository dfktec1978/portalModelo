...
export async function POST(req: Request) {
  try {
    ...
    const bucket = process.env.NEXT_PUBLIC_PROFILE_BUCKET || process.env.NEXT_PUBLIC_PRODUCT_BUCKET || 'product-images';
    const fileName = `profiles/${ownerId}/${Date.now()}_${(file as any).name}`;
