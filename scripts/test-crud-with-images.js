  ...
  try {
    ...
    const { data: createdClassified, error: createError } = await supabase
      ...
      .insert({
        ...
        description: "Este é um teste de CRUD com upload de imagem",
    ...
    console.log(`   Imagem: ${createdClassified.image_urls?.[0]?.substring(0, 50)}...`);
    ...
    // 4. UPDATE - Adicionar outra imagem
    console.log("\n4️⃣  UPDATE - Adicionando segunda imagem...");
    const updatedUrls = [imageUrl, imageUrl]; // Segunda imagem = mesma imagem
    const { error: updateError } = await supabase
      .from("classifieds")
      .update({
        title: "Teste CRUD - iPhone com 2 Imagens (ATUALIZADO)",
    ...
    console.log(`   Imagens: ${updatedClassified?.image_urls?.length || 0}`);
    console.log(`   Preço: R$ ${updatedClassified?.price}`);

    // 5. LIST - Listar com filtros
    console.log("\n5️⃣  LIST - Listando classificados ativos do usuário...");
    const { data: userClassifieds, error: listError } = await supabase
      .from("classifieds")
      .select("*")
    ...
    console.log(`✅ ${userClassifieds?.length || 0} classificado(s) encontrado(s)`);
    ...
    console.log(`   - READ: ✅`);
    console.log(`   - UPDATE com imagens: ✅`);
    console.log(`   - SEARCH: ✅`);
